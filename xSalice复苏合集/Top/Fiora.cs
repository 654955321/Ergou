        ...
        private void LoadMenu()
        {
            ...
            var spellMenu = new Menu("技能菜单", "SpellMenu");
            {
                var qMenu = new Menu("Q菜单", "QMenu");
                {
                    ...
                    qMenu.AddItem(new MenuItem("Q_Gap_Close", "Q 小兵进行突进", true).SetValue(true));
                    spellMenu.AddSubMenu(qMenu);
                }
                var wMenu = new Menu("W菜单", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("W_Incoming", "W 总是抵挡普攻", true).SetValue(true));
                    wMenu.AddItem(new MenuItem("W_Tower", "敌方塔下禁用W", true).SetValue(true));
                    wMenu.AddItem(new MenuItem("W_minion", "W 抵挡小兵", true).SetValue(false));
                    spellMenu.AddSubMenu(wMenu);
                }
                var eMenu = new Menu("E菜单", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Reset", "E 自动攻击 复位", true).SetValue(true));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R菜单", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_If_HP", "R 如果自己血量 <=", true).SetValue(new Slider(20)));

                    //evading spells
                    var dangerous = new Menu("躲避危险", "Dodge Dangerous");
                    {
                        foreach (var hero in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy))
                        {
                            ...
                            dangerous.SubMenu(hero.ChampionName).AddItem(new MenuItem(hero.Spellbook.GetSpell(SpellSlot.Q).Name + "R_Dodge", hero.Spellbook.GetSpell(SpellSlot.Q).Name+ "使用 R_躲避", true).SetValue(false));
                            dangerous.SubMenu(hero.ChampionName).AddItem(new MenuItem(hero.Spellbook.GetSpell(SpellSlot.W).Name + "R_Dodge", hero.Spellbook.GetSpell(SpellSlot.W).Name+ "使用 R_躲避", true).SetValue(false));
                            dangerous.SubMenu(hero.ChampionName).AddItem(new MenuItem(hero.Spellbook.GetSpell(SpellSlot.E).Name + "R_Dodge", hero.Spellbook.GetSpell(SpellSlot.E).Name+ "使用 R_躲避", true).SetValue(false));
                            dangerous.SubMenu(hero.ChampionName).AddItem(new MenuItem(hero.Spellbook.GetSpell(SpellSlot.R).Name + "R_Dodge", hero.Spellbook.GetSpell(SpellSlot.R).Name+ "使用 R_躲避", true).SetValue(false));
                        }
            ...
            {
                combo.AddItem(new MenuItem("Combo_mode", "连招 模式", true).SetValue(new StringList(new[] { "正常", "Q-AA-Q-AA-大招" })));
            ...
            var customMenu = new Menu("自定义 键位启用栏 显示", "Custom Perma Show");
            {
                ...
                customMenu.AddItem(myCust.AddToMenu("补刀 启用: ", "LastHitKey"));
