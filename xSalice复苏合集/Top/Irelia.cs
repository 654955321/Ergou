    ...
    class Irelia : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var spellMenu = new Menu("技能菜单", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Min_Distance", "使用 Q 最小范围", true).SetValue(new Slider(300, 0, 600)));
                    qMenu.AddItem(new MenuItem("Q_Gap_Close", "Q 小兵进行突进", true).SetValue(true));
                    qMenu.AddItem(new MenuItem("Q_Under_Tower", "Q 敌人塔下敌人", true).SetValue(false));
                    spellMenu.AddSubMenu(qMenu);
                }

                var eMenu = new Menu("E菜单", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Only_Stun", "保存 E 进行眩晕", true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Running", "E 逃跑中的敌人", true).SetValue(true));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R菜单", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_If_HP", "R 如果 HP <=", true).SetValue(new Slider(20)));
            ...
            var farm = new Menu("清线", "LaneClear");
            {
                ...
                farm.AddItem(new MenuItem("UseQFarm_Tower", "禁用 Q 在塔下", true).SetValue(true));
            ...
            var miscMenu = new Menu("杂项", "Misc");
            {
                ...
                miscMenu.AddItem(new MenuItem("QE_Interrupt", "使用 Q/E 中断法术", true).SetValue(true));
