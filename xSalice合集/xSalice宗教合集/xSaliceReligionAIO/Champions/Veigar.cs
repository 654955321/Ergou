    ...
    class Veigar : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var key = new Menu("按 键", "Keys");
            {
                ...
                key.AddItem(new MenuItem("HarassActiveT", "骚 扰 (自动)!", true).SetValue(new KeyBind("Y".ToCharArray()[0], KeyBindType.Toggle)));
                key.AddItem(new MenuItem("LastHitQQ", "Q丨最后一下", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("LastHitQQ2", "自动Q丨最后一下", true).SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Toggle)));
                key.AddItem(new MenuItem("wPoke", "使用W丨尽在对面眩晕时", true).SetValue(new KeyBind("N".ToCharArray()[0], KeyBindType.Toggle)));
                key.AddItem(new MenuItem("escape", "逃 跑", true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                menu.AddSubMenu(key);
            }
            ...
            var combo = new Menu("连 招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("DFGMode", "冥火模式", true).SetValue(new StringList(new[] {"连 招", "冥火+R"})));
            ...
            var misc = new Menu("杂 项", "Misc"); { 
                ...
                misc.AddItem(new MenuItem("UseGap", "使用 W丨防突进", true).SetValue(true));
                ...
                misc.AddSubMenu(new Menu("禁止对()使用R ", "DontUlt"));
