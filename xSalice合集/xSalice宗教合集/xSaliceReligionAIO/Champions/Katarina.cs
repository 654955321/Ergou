    ...
    class Katarina : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var killSteal = new Menu("抢人头", "KillSteal");
            {
                ...
                killSteal.AddItem(new MenuItem("dfgKS", "使用 冥火 抢人头", true).SetValue(true));
                killSteal.AddItem(new MenuItem("rCancel", "取消 R 抢人头", true).SetValue(false));
                killSteal.AddItem(new MenuItem("KS_With_E", "禁止自动E抢头!", true).SetValue(new KeyBind("H".ToCharArray()[0], KeyBindType.Toggle)));
                //add to menu
