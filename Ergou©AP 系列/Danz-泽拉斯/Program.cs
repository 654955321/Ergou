...
namespace Xerath___The_Magus_Ascendant
{
  class Program
  {
    ...
    static void Game_OnGameLoad(EventArgs args)
    {
      ...
      SpellList.Add(E);
      SpellList.Add(R);
      Config = new Menu("Danz-泽拉斯", "xerath_menu", true);
      ...
      Config.SubMenu("Combo").AddItem(new MenuItem("UseECombo", "使用 E")).SetValue(false);
      Config.SubMenu("Combo").AddItem(new MenuItem("UseRCombo", "使用 R 如果 可击杀")).SetValue(true);
      ...
      Config.AddSubMenu(new Menu("清线", "Farming"));
      Config.SubMenu("Farming").AddItem(new MenuItem("QClear", "使用 Q")).SetValue(true);
      Config.SubMenu("Farming").AddItem(new MenuItem("WClear", "使用 W")).SetValue(true);
      Config.SubMenu("Farming").AddItem(new MenuItem("QClearmana", "最低蓝量 % Q 清线")).SetValue(new Slider(30, 0, 100));
      Config.SubMenu("Farming").AddItem(new MenuItem("WClearmana", "最低蓝量 % W 清线")).SetValue(new Slider(50, 0, 100));

      Config.AddSubMenu(new Menu("R 设置", "ROptions"));
      Config.SubMenu("ROptions").AddItem(new MenuItem("BlockMove", "禁止移动直到使用完R")).SetValue(true);
      Config.SubMenu("ROptions").AddItem(new MenuItem("castR", "强制 R 使用").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
      Config.SubMenu("ROptions").AddSubMenu(new Menu("R延迟", "Delays"));
      for (int i = 1; i <= 3; i++)
        Config.SubMenu("ROptions").SubMenu("Delays").AddItem(new MenuItem("Delays" + i, "延迟" + i).SetValue(new Slider(0, 1500, 0)));
      ...
      Config.SubMenu("KS").AddItem(new MenuItem("UseRKS", "使用 R")).SetValue(false);

      Config.AddSubMenu(new Menu("中断技能", "Interrupter"));
      Config.SubMenu("Interrupter").AddItem(new MenuItem("EInterrupt", "中断技能 E")).SetValue(true);

      Config.AddSubMenu(new Menu("防止突进", "AntiGapCloser"));
      Config.SubMenu("AntiGapCloser").AddItem(new MenuItem("EGC", "防止突进 E")).SetValue(true);

      Config.AddSubMenu(new Menu("显示", "Drawings"));
      Config.SubMenu("Drawings").AddItem(new MenuItem("drawEnable", "启用所有显示")).SetValue(true);
      Config.SubMenu("Drawings").AddItem(new MenuItem("drawpred", "显示 击杀提示")).SetValue(true);
      ...
      Config.SubMenu("Drawings").AddItem(new MenuItem("drawkill", "显示大招对敌方英雄血量伤害(可能导致 FPS 降低, 需要修复!)")).SetValue(false);
