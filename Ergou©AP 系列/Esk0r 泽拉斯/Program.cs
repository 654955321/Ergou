...
namespace Xerath
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            Config = new Menu("Esk0r 泽拉斯", ChampionName, true);
            ...
            Config.SubMenu("R").AddItem(new MenuItem("EnableRUsage", "自动使用R").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("rMode", "R 模式").SetValue(new StringList(new[] { "正常", "自定义延迟", "优先"})));
            Config.SubMenu("R").AddItem(new MenuItem("rModeKey", "R 模式 切换").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            ...
            Config.SubMenu("R").AddItem(new MenuItem("PingRKillable", "本地击杀提示").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("BlockMovement", "阻止右键点击（禁止移动）直到R施放完").SetValue(false));
            Config.SubMenu("R").AddItem(new MenuItem("OnlyNearMouse", "只对鼠标附近的敌人施放R").SetValue(false));
            ...
            Config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("UseQFarm", "使用 Q").SetValue(
                        new StringList(new[] { "控 线", "清 线", "两者", "禁用"}, 2)));
            Config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("UseWFarm", "使用 W").SetValue(
                        new StringList(new[] { "控 线", "清 线", "两者", "禁用" }, 1)));
            Config.SubMenu("Farm")
                .AddItem(
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("AutoEGC", "自动E 突进者").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("UseVHHC", "使用 非常高的命中率").SetValue(true));

            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterR","显示 3xR 后的伤害").SetValue(true);
