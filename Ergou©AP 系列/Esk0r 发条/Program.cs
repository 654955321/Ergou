...
namespace Orianna
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("Esk0r 发条", "Orianna", true);
            TargetSelector.AddToMenu(Config.SubMenu("目标选择"));
            Orbwalker = new Orbwalking.Orbwalker(Config.SubMenu("走砍"));

            #region Combo
            Config.SubMenu("连招").AddItem(new MenuItem("UseQCombo", "使用 Q").SetValue(true));
            Config.SubMenu("连招").AddItem(new MenuItem("UseWCombo", "使用 W").SetValue(true));
            Config.SubMenu("连招").AddItem(new MenuItem("UseECombo", "使用 E").SetValue(true));
            Config.SubMenu("连招").AddItem(new MenuItem("UseRCombo", "使用 R").SetValue(true));
            Config.SubMenu("连招").AddItem(new MenuItem("UseRNCombo", "使用 R | 目标命中").SetValue(new StringList(new string[]{"1 目标", "2 目标", "3 目标", "4 目标", "5 目标"}, 0)));
            Config.SubMenu("连招")
                .AddItem(
            ...
            #region Misc
            Config.SubMenu("杂项").AddItem(new MenuItem("AutoW", "自动W | 目标命中").SetValue(new StringList(new string[] { "禁用", ">=1 目标", ">=2 目标", ">=3 目标", ">=4 目标", ">=5 目标" }, 2)));
            Config.SubMenu("杂项").AddItem(new MenuItem("AutoR", "自动R | 目标命中").SetValue(new StringList(new string[] { "禁用", ">=1 目标", ">=2 目标", ">=3 目标", ">=4 目标", ">=5 目标" }, 3)));
            Config.SubMenu("杂项").AddItem(new MenuItem("AutoEInitiators", "自动 E 护盾保护").SetValue(true));
            ...
            Config.SubMenu("杂项").AddItem(new MenuItem("InterruptSpells", "使用R 中断法术").SetValue(true));
            Config.SubMenu("杂项").AddItem(new MenuItem("BlockR", "如果不能命中敌方禁用R").SetValue(false));
            #endregion

            #region Harass
            //Harass menu:
            Config.SubMenu("骚扰").AddItem(new MenuItem("UseQHarass", "使用 Q").SetValue(true));
            Config.SubMenu("骚扰").AddItem(new MenuItem("UseWHarass", "使用 W").SetValue(false));
            Config.SubMenu("骚扰").AddItem(new MenuItem("HarassManaCheck", "禁用骚扰|魔法值 < %").SetValue(new Slider(0, 0, 100)));
            Config.SubMenu("骚扰")
                .AddItem(
            ...
            Config.SubMenu("清线")
                .AddItem(
                    new MenuItem("UseEFarm", "使用 E").SetValue(
                        new StringList(new[] { "控线", "清线", "同时", "禁用"}, 1)));
            Config.SubMenu("清线").AddItem(new MenuItem("LaneClearManaCheck", "禁用清线|魔法值< %").SetValue(new Slider(0, 0, 100)));
            ...
            #region Drawings
            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterR", "显示组合连招伤害").SetValue(true);
            ...
            Config.SubMenu("范围")
                .AddItem(
                    new MenuItem("QOnBallRange", "显示 球 落地点").SetValue(new Circle(true, Color.FromArgb(150, Color.DodgerBlue))));
            Config.SubMenu("范围")
