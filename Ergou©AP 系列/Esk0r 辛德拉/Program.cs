...
//            DFG = Utility.Map.GetMap().Type == Utility.Map.MapType.TwistedTreeline ? new Items.Item(3188, 750) : new Items.Item(3128, 750);
            ...
            //Create the menu
            Config = new Menu("Esk0r 辛德拉", ChampionName, true);
            ...
            Config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("UseEFarm", "使用 E").SetValue(
                        new StringList(new[] { "控线", "清线", "同时", "禁用" }, 3)));
            Config.SubMenu("Farm")
                .AddItem(
            ...
            Config.SubMenu("Misc")
                .AddItem(
                    new MenuItem("CastQE", "QE 鼠标方向").SetValue(new KeyBind("T".ToCharArray()[0],
                        KeyBindType.Press)));
            Config.SubMenu("Misc").AddSubMenu(new Menu("禁用大招", "DontUlt"));

            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.Team != Player.Team))
                Config.SubMenu("Misc")
                    .SubMenu("DontUlt")
                    .AddItem(new MenuItem("DontUlt" + enemy.BaseSkinName, "禁用大招敌人名字：" + enemy.BaseSkinName).SetValue(false));
