...
namespace BrianSharp
{
    internal class Program
    {
        ...
        private static void OnStart(EventArgs args)
        {
            ...
            MainMenu = new Menu("Brian 合集", "BrianSharp", true);
            var infoMenu = new Menu("信息", "Info");
            {
                infoMenu.AddItem(new MenuItem("Author", "汉化: Ergou"));
                infoMenu.AddItem(new MenuItem("Paypal", "QQ群: 361630847"));
            ...
            NewInstance(plugin);
            Helper.AddBool(
                MainMenu.SubMenu(Player.ChampionName + "_Plugin").SubMenu("Misc"), "UsePacket", "使用封包");
