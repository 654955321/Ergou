...
namespace BrianSharp.Plugin
{
    internal class Yasuo : Helper
    {
        ...
        public Yasuo()
        {
            ...
            Q2.MinHitChance = HitChance.High;
            ...
            var champMenu = new Menu("Plugin", Player.ChampionName + "_Plugin");
            {
                var comboMenu = new Menu("连招", "Combo");
                {
                    ...
                    AddBool(comboMenu, "EDmg", "-> 处理伤害 (必须Q3)");
                    AddBool(comboMenu, "EGap", "-> 突进追击");
                    AddSlider(comboMenu, "EGapRange", "-> 启用突进追击|与敌人距离差", 300, 1, 475);
                    AddBool(comboMenu, "EGapTower", "-> 敌方塔下使用", false);
                    AddBool(comboMenu, "R", "使用 R");
                    AddBool(comboMenu, "RDelay", "-> 延迟设置");
                    AddSlider(comboMenu, "RDelayTime", "--> 延迟 (ms)", 200, 200, 400);
                    AddSlider(comboMenu, "RHpU", "-> 使用R|敌人血量低于", 50);
                    AddSlider(comboMenu, "RCountA", "-> 使用R|敌人数量大于", 2, 1, 5);
                    champMenu.AddSubMenu(comboMenu);
                }
                var harassMenu = new Menu("骚扰", "Harass");
                {
                    AddKeybind(harassMenu, "AutoQ", "自动 Q", "H", KeyBindType.Toggle, true);
                    AddBool(harassMenu, "AutoQ3", "-> 使用 Q3", false);
                    AddBool(harassMenu, "AutoQTower", "-> 敌方塔下使用", false);
                    AddBool(harassMenu, "Q", "使用 Q");
                    AddBool(harassMenu, "Q3", "-> 使用 Q3");
                    AddBool(harassMenu, "QTower", "-> 敌方塔下使用", false);
                    AddBool(harassMenu, "QLastHit", "-> 补刀 (Q1/Q2)");
                    champMenu.AddSubMenu(harassMenu);
                }
                var clearMenu = new Menu("清线", "Clear");
                {
                    AddBool(clearMenu, "Q", "使用 Q");
                    AddBool(clearMenu, "Q3", "-> 使用 Q3");
                    AddBool(clearMenu, "E", "使用 E");
                    AddBool(clearMenu, "ETower", "-> 敌方塔下使用", false);
                ...
                var lastHitMenu = new Menu("补刀", "LastHit");
                {
                    ...
                    AddBool(lastHitMenu, "Q3", "-> 使用 Q3");
                    AddBool(lastHitMenu, "E", "使用 E");
                    AddBool(lastHitMenu, "ETower", "-> 敌方塔下使用", false);
                    champMenu.AddSubMenu(lastHitMenu);
                }
                var fleeMenu = new Menu("逃跑", "Flee");
                {
                    AddBool(fleeMenu, "E", "使用 E");
                    AddBool(fleeMenu, "EStackQ", "-> 逃跑时堆叠 Q");
                ...
                var miscMenu = new Menu("杂项", "Misc");
                {
                    ...
                    AddKeybind(miscMenu, "StackQ", "自动堆叠 Q", "Z", KeyBindType.Toggle);
                    AddBool(miscMenu, "StackQDraw", "-> 显示 文本信息");
        ...
        private Obj_AI_Hero QCirTarget
        {
            get
            {
                ...
                return Player.Distance(pos) < 50
                    ? TargetSelector.GetTarget(QCirWidth, TargetSelector.DamageType.Physical, true, null, pos)
                    : null;
            }
        }
        ...
        private void OnPossibleToInterrupt(Obj_AI_Hero unit, InterruptableSpell spell)
        {
            ...
            if (Player.IsDashing())
            {
                ...
                if (Player.Distance(pos) < 50 && unit.Distance(pos) <= QCirWidth)
                {
        ...
        private void Fight(string mode)
        {
                ...
                if (GetValue<bool>(mode, "R") && R.IsReady())
                {
                    var obj = (from enemy in HeroManager.Enemies.Where(i => R.IsInRange(i) && CanCastR(i))
                        ...
                        where
                            ...
                            (sub.Count > 1 && sub.Any(i => i.HealthPercent < GetValue<Slider>(mode, "RHpU").Value)) ||
            ...
            if (GetValue<bool>(mode, "Q") && Q.IsReady())
            {
                     ...
                     (!UnderTower(Player.ServerPosition) || GetValue<bool>(mode, "QTower"))))
                {
                    if (Player.IsDashing())
                    {
                        if (QCirTarget != null && Q.Cast(QCirTarget.ServerPosition, PacketCast))
                        {
                            return;
                        }
                    }
                        ...
                        (!HaveQ3 || GetValue<bool>("Clear", "Q3")))
                    {
                        obj = (from i in minionObj
                            ...
                            where
                                ...
                                sub.Any(a => Q.IsKillable(a)) || sub.Count(a => MinionManager.IsMinion(a)) > 1
            ...
            if (GetValue<bool>("Clear", "Q") && Q.IsReady() && (!HaveQ3 || GetValue<bool>("Clear", "Q3")))
            {
                    ...
                    if (Player.Distance(Player.GetDashInfo().EndPos) < 100)
                    {
                        var minionObj =
                            MinionManager.GetMinions(
                                ...
                                .Cast<Obj_AI_Minion>()
                                .ToList();
                        if ((minionObj.Any(i => i.Team == GameObjectTeam.Neutral || Q.IsKillable(i)) ||
                             minionObj.Count(i => MinionManager.IsMinion(i)) > 1) &&
                            Q.Cast(minionObj.First().ServerPosition, PacketCast))
                        {
                            return;
                        }
                    }
                ...
                else
                {
                    var minionObj = MinionManager.GetMinions(
                        (!HaveQ3 ? Q : Q2).Range, MinionTypes.All, MinionTeam.NotAlly, MinionOrderTypes.MaxHealth);
                    if (minionObj.Any())
                    {
                        if (!HaveQ3)
                        {
                        ...
                        var pos = (!HaveQ3 ? Q : Q2).GetLineFarmLocation(minionObj, 20);
        ...
        private void Flee()
        {
            ...
            if (GetValue<bool>("Flee", "EStackQ") && Q.IsReady() && !HaveQ3 && Player.IsDashing())
            {
                ...
                if (Player.Distance(Player.GetDashInfo().EndPos) < 100)
                {
                    var minionObj = MinionManager.GetMinions(
                        Player.GetDashInfo().EndPos.To3D(), QCirWidth, MinionTypes.All, MinionTeam.NotAlly);
                    if (minionObj.Any() && Q.Cast(minionObj.First().ServerPosition, PacketCast))
                    {
                        return;
                    }
                }
        ...
        private void KillSteal()
        {
            ...
            if (GetValue<bool>("KillSteal", "Q") && Q.IsReady())
            {
                if (Player.IsDashing())
                {
                    ...
                    if (target != null && Q.IsKillable(target) && Q.Cast(target.ServerPosition, PacketCast))
                    {
                        return;
                    }
                }
                ...
                if (target != null &&
                    (CanKill(target, GetEDmg(target)) ||
                     (GetValue<bool>("KillSteal", "Q") && Q.IsReady(50) &&
                      CanKill(target, GetEDmg(target) + Q.GetDamage(target)))) && E.CastOnUnit(target, PacketCast))
                {
                    return;
                }
            }
        ...
        {
            return Player.ServerPosition.Extend(target.ServerPosition, E.Range + target.BoundingRadius);
        }
